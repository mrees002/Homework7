...
data %>%
   group_by(GOTV_call) %>%
   filter(n() > 1) %>%
   summarise(Count = n(), prop_vote1996 = mean(voted1996))
...
As can be seen above, the proportion of people who voted in 1998 and were called was greater among the group who voted in 1996, showing that this variable is confounding. The confidence interval for this difference in proportions test resulted in a confidence interval of  (-0.43, -0.39), very convincing evidence of a difference in those who voted in the previous election.
...
data %>%
   group_by(GOTV_call) %>%
   filter(n() > 1) %>%
   summarise(Count = n(), Average_Age = mean(AGE))
...
t.test(AGE ~ voted1998, data=data)
```

This demonstrates that the average age of those who were called and voted in 1998 was higher than those who didn't, this shows that age is another confounding variable. A confidence interval for a difference in means test resulted in a confidence interval of (-11.2, -9.8), another case of very convincing evidence of a connection between age and likelihood of voting.
...
data %>%
   group_by(GOTV_call) %>%
   filter(n() > 1) %>%
   summarise(Count = n(), Prop_majparty = mean(MAJORPTY))
...
prop.test(voted1998 ~ MAJORPTY, data=data, success = 1)
```

This chart above shows that the proportion of people who were called and voted in 1998 was higher among those that were registered member of either of the two major political parties, showing this is another confounding variable. A confidence interval for difference in proportions resulted in an interval of (-0.15, -0.11), showing clearly that there is a difference in voting percentage for those registered to a major party and those not.
...
#voted1996
mean(call_matched$voted1996[call_matched$GOTV_call == 1], na.rm = TRUE) - 
mean(call_matched$voted1996[call_matched$GOTV_call == 0], na.rm = TRUE)

#AGE
mean(call_matched$AGE[call_matched$GOTV_call == 1], na.rm = TRUE) - 
mean(call_matched$AGE[call_matched$GOTV_call == 0], na.rm = TRUE)

#MAJORPTY
mean(call_matched$MAJORPTY[call_matched$GOTV_call == 1], na.rm = TRUE) - 
mean(call_matched$MAJORPTY[call_matched$GOTV_call == 0], na.rm = TRUE)
```

We can see above that the matched dataset has negligible differences between the groups that were called and the groups that were not called to vote in the 1998 election, with the first difference being if the person voted in 1996, the second being age, and the last being registered to a majority party.
